<h3 class="text-muted">Edit Data Pasok</h3>
<hr>
<form method="post">
	<?php  
		$id = @$_GET['id'];
		$show = $db->prepare("SELECT * FROM pasok WHERE id_pasok = ?");
		$show->execute(array($id));
		$data = $show->fetch(PDO::FETCH_LAZY);
	?>
	<div class="form-group col-sm-6">
		<label>ID Pasok</label>
		<input type="text" value="<?php echo $data->id_pasok; ?>" name="id" required class="form-control" readonly>
	</div>
	<div class="form-group col-sm-6">
		<label>Nama Distributor</label>
		<select class="form-control" name="distributor">
			<option value=""> ==> Pilih Distributor <== </option>
		<?php  
			$show = $db->prepare("SELECT * FROM distributor");
			$show->execute();
			while ($showed = $show->fetch(PDO::FETCH_LAZY)) {
				?>
					<option value="<?php echo $showed->id_distributor; ?>"><?php echo $showed->nama_distributor; ?></option>
				<?php
			}
		?>
			
		</select>
	</div>
	<div class="form-group col-sm-6">
		<label>Nama Buku</label>
		<select class="form-control" name="buku">
			<option value=""> ==> Pilih Judul Buku <== </option>
		<?php  
			$show = $db->prepare("SELECT * FROM buku");
			$show->execute();
			while ($showed = $show->fetch(PDO::FETCH_LAZY)) {
				?>
					<option value="<?php echo $showed->id_buku; ?>"><?php echo $showed->judul; ?></option>
				<?php
			}
		?>
			
		</select>
	</div>
	<div class="form-group col-sm-3">
		<label>Jumlah</label>
		<input type="text" value="<?php echo $data->jumlah; ?>" name="jumlah" required class="form-control">
	</div>
	<div class="form-group col-sm-3">
		<label>Tanggal</label>
		<input type="date" value="<?php echo $data->tanggal; ?>" name="tanggal" required class="form-control">
	</div>
	<div class="form-group col-sm-6">
		<button name="edit" value="Edit" class="btn btn-primary">Edit <i class="fa fa-send"></i></button>
		<a href=".?page=pasok" class="btn btn-danger">Cancel</i></a>
	</div>
</form>
<?php  
	$buku = @$_POST['buku'];
	$distributor = @$_POST['distributor'];
	$jumlah = @$_POST['jumlah'];
	$tanggal = @$_POST['tanggal'];
	try {
		if (@$_POST['edit']) {
			$edit = $db->prepare("UPDATE pasok SET id_distributor = ?, id_buku = ?, jumlah = ?, tanggal = ? WHERE id_pasok = $id");
			$array = array($distributor, $buku, $jumlah, $tanggal);
			$edit->execute($array);
			if ($edit->rowCount()>0) {
				$update = $db->prepare("UPDATE buku SET stok = stok + ? WHERE id_buku = $buku");
				$update->execute(array($jumlah));
				?>
					<script type="text/javascript">
						window.location.href=".?page=pasok";
					</script>
				<?php
			}
		}
	} catch (Exception $e) {
		echo "Failed " .$e->getMessage();
	}
?>