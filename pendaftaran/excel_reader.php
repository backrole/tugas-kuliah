...
// readData(rootStartBlock)
	...
	function __readPropertySets(){
		...
		while ($offset < strlen($this->entry)) {
			...
			$nameSize = ord($d[SIZE_OF_NAME_POS]) | (ord($d[SIZE_OF_NAME_POS+1]) 
				<< 8);
			$type = ord($d[TYPE_POS]);
...
class Spreadsheet_Excel_Reader {
	...
	function dumpHexData($data, $pos, $length) {
		$info = "";
		for ($i = 0; $i <= $length; $i++) {
			$info .= ($i==0?"":" ") . $this->myHex(ord($data[$pos + $i])) . (ord($data[$pos + 
				$i])>31? "[" . $data[$pos + $i] . "]":'');
		}
		return $info;
	}
...
function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
	...
	if ($col_letters) {
		...
		for($i=1;$i<=$this->colcount($sheet);$i++) {
			...
			$out .= "\n\t\t<th style=\"$style\">" . strtoupper($this->colindexes[$i]) . 
			"</th>";
		}
		$out .= "</tr></thead>\n";
	}
...
// Read in the crun
			...
			if ($extendedString) {
	// Read in cchExtRst
				$extendedRunLength = $this->_GetInt4d($data, 
					$spos);
				$spos += 4;
			}
...
$retstr = $newstr;
$len = min($charsLeft * 2, 
$limitpos - $spos); // min($charsLeft, $conlength);
$retstr .= substr($data, $spos, $len);
$charsLeft -= $len/2;
$asciiEncoding = false;
}
...
if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
	...
	if (ord($data[$pos+8]) == 0){
		$formatString = substr($data, $pos+9, 
			$numchars);
	} else {
		$formatString = substr($data, $pos+9, 
			$numchars*2);
...
$fontIndexCode = (ord($data[$pos+4]) | 
	ord($data[$pos+5]) << 8) - 1;
$fontIndexCode = max(0,$fontIndexCode);
$indexCode = ord($data[$pos+6]) | ord($data[$pos+7]) 
<< 8;
$alignbit = ord($data[$pos+10]) & 3;
$bgi = (ord($data[$pos+22]) | ord($data[$pos+23]) << 
	8) & 0x3FFF;
$bgcolor = ($bgi & 0x7F);
...
// read BOF
	...
	while($cont) {
		...
		switch ($code) {
			...
			$colLast  = ord($data[$spos + $length - 2]) | 
			ord($data[$spos + $length - 1])<<8;
...
// http://code.google.com/p/php-excelreader/issues/detail?id=4
	$previousRow = $row;
	$previousCol = $column;
...
$cols = ($length / 2) - 3;
for ($c = 0; $c < $cols; $c++) {
	$xfindex = ord($data[$spos + 4 + ($c * 2)]) | 
	ord($data[$spos + 5 + ($c * 2)])<<8;
...
$row  = ord($data[$spos]) | ord($data[$spos+1])<<8;
$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
$this->addcell($row, $column, substr($data, $spos + 8, 
	ord($data[$spos + 6]) | ord($data[$spos + 7])<<8));
break;
...
for ($coli = $colfrom; $coli <= $colto; $coli++) {
	$this->colInfo[$this->sn][$coli+1] = Array('width' => 
		$cw, 'xf' => $cxf, 'hidden' => ($co & 0x01), 'collapsed' => ($co & 0x1000) >> 12);
}
break;
default:
break;
...
function _getCellDetails($spos,$numValue,$column) {
	...
	if ($type == 'date') {
// See http://groups.google.com/group/php-excel-readerdiscuss/browse_frm/thread/9c3f9790d12d8e10/f2045c2369ac79de
		$rectype = 'date';
...
$string = date ($format, mktime($hours, $mins, $secs, 
	...
	$rectype = 'number';
	$formatted = $this->_format_value($format, $numValue, 
		$formatIndex);
