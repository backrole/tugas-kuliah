<h5 class="blue-text margin-left:10px;">
    <blockquote><strong>Edit Data Barang</strong></blockquote>
  </button>
</h5>
<?php  
    ...
    <form class="col s12" method="post">
      ...
      <div class="row">
        <blockquote style="margin-left:10px;">
          <button class="btn waves-effect waves-light" value="simpan" name="simpan">Simpan<i class="mdi-content-save left"></i></button>&nbsp;&nbsp;&nbsp;
          <button class="btn waves-effect waves-light red" value="batal" name="batal">Batal</button>
        </blockquote>
      </div>
    </form>
  </div>
    ...
    try {
        if (@$_POST['simpan']) {
            ...
            if ($update->rowCount()>0) {
              $upd = $db->prepare("UPDATE data_persediaan SET nama_barang = ? WHERE id_masuk = '$kode'");
              $upd->execute(array($nama));
                ?>
                    <script>
